...
const listDescription =
  "This folder contains extensions that are loaded by the SPFxExtensions application.";
...
async function doEnsureSPFxExtensionsFolder(sp: SPFI): Promise<void> {
  try {
    await sp.web.lists.getByTitle(SPFX_EXTENSIONS_FOLDER)();
  } catch (error) {
    if (error instanceof HttpRequestError && error.status === 404) {
      await sp.web.lists.add(SPFX_EXTENSIONS_FOLDER, listDescription, 101, false, {
        Hidden: true,
      });
      return;
    }
...
export async function deleteRootFolderRecursively(
  sp: SPFI,
  ...
  const allFiles = await sp.web.lists
    ...
    .items.filter(`startswith(FileDirRef,'${relativePath}') and FSObjType eq 0`)
    .select("Id")();
  ...
  const appCollectionFolder = sp.web.lists
    .getByTitle(SPFX_EXTENSIONS_FOLDER)
    .rootFolder.folders.getByUrl(rootFolderName);
