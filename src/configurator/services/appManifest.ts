...
export async function getAppManifest(sp: SPFI, appName: string) {
  ...
  const manifestQuery = sp.web.getFileByUrl(
    `${SPFX_EXTENSIONS_FOLDER}/${appName}/${MANIFEST_NAME}`
  );
  ...
  if (!fileExists) {
    ...
    await sp.web.lists
      ...
      .rootFolder.folders.getByUrl(appName)
      .files.addUsingPath(MANIFEST_NAME, JSON.stringify(EMPTY_APP_MANIFEST));
    logGenericCoreInfo(
      `Created ${MANIFEST_NAME} in ${SPFX_EXTENSIONS_FOLDER}/${appName} folder in ${webUrl}`
    );
    return result;
  }
  try {
...
export async function updateAppManifest(
  ...
  try {
    ...
    const webUrl = getWebUrlFromSP(sp);
    logGenericCoreError(
      `Error while updating ${MANIFEST_NAME} in ${SPFX_EXTENSIONS_FOLDER}/${appName} folder in ${webUrl}`,
      error
    );
    return false;
  }
