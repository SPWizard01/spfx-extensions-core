import { type ICustomStoreParams, IDBStorage, type PNPCacheValue } from './idbStorage';
...
export async function staleWhileRevalidateWithExpiry<T>(
    ...
    const expiryDate = expiry && typeof Date === 'object' ? expiry : new Date(new Date().getTime() + 1000 * 60 * 10); // 10 minutes

    if (!indexedDBStorageInstance.indexedDBError) {
        const value = <PNPCacheValue<T>>await indexedDBStorageInstance.getItem(key);
        const isExpired = value.expiry <= new Date();

        if (value && value.indexedDBCache && !isExpired) {
            // update cache once we have a result
            p.then((u) => {
                const response: PNPCacheValue = { expiry: expiryDate, data: u, indexedDBCache: 1 };
    ...
    // Set Cache
    if (!indexedDBStorageInstance.indexedDBError) {
        const data: PNPCacheValue = { expiry: expiryDate, data: result, indexedDBCache: 1 };
