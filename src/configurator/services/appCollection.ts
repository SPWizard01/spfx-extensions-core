...
export async function removeAppCollection(sp: SPFI, collectionName: string) {
  ...
  try {
    ...
    // }
    logGenericCoreError(
      `Error while removing ${collectionName} from ${SPFX_EXTENSIONS_FOLDER} folder in ${getWebUrlFromSP(sp)}`,
      error
    );
  }
  return false;
...
export async function updateAppCollectionConfig(
  sp: SPFI,
  appCollection: SPFxExtensionCollectionManifest
) {
  contextCollectionConfigUpdating.value = true;
  await getAppCollectionConfig(sp);
  const webUrl = getWebUrlFromSP(sp);
  const manifestQuery = sp.web.getFileByUrl(
    `${SPFX_EXTENSIONS_FOLDER}/${APPCOLLECTION_MANIFEST_NAME}`
  );
  try {
    ...
    logGenericCoreError(
      `Error while updating ${APPCOLLECTION_MANIFEST_NAME} in ${SPFX_EXTENSIONS_FOLDER} folder in ${webUrl}`,
      error
    );
    return false;
  } finally {
...
export async function ensureAppCollectionNestedPath(
  sp: SPFI,
  appName: string,
  nestedPath: string[]
) {
  ...
  for (const subPath of nestedPath) {
    ...
    if (!subPathCheck.some((f) => f.Name.toLowerCase() === subPath.toLowerCase())) {
      logGenericCoreInfo(
        `Creating subfolder ${processingPath} in ${SPFX_EXTENSIONS_FOLDER}/${appName} folder in ${webUrl}`
      );
