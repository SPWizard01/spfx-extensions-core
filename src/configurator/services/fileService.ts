...
export async function* addFiles(sp: SPFI, appName: string, fileContents: FileContents[]) {
  ...
  for (const file of fileContents) {
    ...
    const fileNameWithoutFolder = hasFolder
      ? file.fileName.substring(pathIdxBeforeFile + 1)
      : file.fileName;
    const subpathBeforeFile = file.fileName.substring(0, pathIdxBeforeFile);
    const fullPath = `${SPFX_EXTENSIONS_FOLDER}/${appName}${
      subpathBeforeFile ? `/${subpathBeforeFile}` : ``
    }`;
