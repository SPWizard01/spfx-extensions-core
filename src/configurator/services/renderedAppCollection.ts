...
export async function getAllAppItems(sp: SPFI, allAppCollections: string[], enabledAppCollections: string[]) {
    ...
    for (const appCollection of allAppCollections) {
        batchedWeb.getFileByUrl(`${SPFX_EXTENSIONS_FOLDER}/${appCollection}/${MANIFEST_NAME}`).getText()
            .then((blob) => {
